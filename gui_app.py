...
def get_resource_path(relative_path):
    ...
    possible_paths = [
        ...
        os.path.join(os.path.dirname(os.path.abspath(__file__)), relative_path) if not getattr(sys, 'frozen', False) else None
    ]
...
if os.path.exists(os.path.join(base_path, '_internal')):
    sys.path.insert(0, os.path.join(base_path, '_internal'))
...
from PyQt6.QtWidgets import (
    ...
    QTextEdit, QGraphicsOpacityEffect, QScrollArea, QSizePolicy, QLayout
)
from PyQt6.QtGui import QMovie, QLinearGradient
...
class LoginDialog(QDialog):
    ...
    def __init__(self, user_manager):
        ...
        # 创建主容器
        main_widget = QWidget()
        main_widget.setFixedSize(400, 360)
        ...
        pixmap_path = get_resource_path(os.path.join("RESOURCE", "1772909182983.png"))
        ...
        title_label.setFont(QFont('微软雅黑', 18, QFont.Weight.DemiBold))
        ...
        self.login_button.setStyleSheet("""
            QPushButton {
                ...
                font-family: '微软雅黑', 'Times New Roman';
                font-weight: 600;
                box-shadow: 0 4px 6px -1px rgba(102, 126, 234, 0.4), 0 2px 4px -1px rgba(118, 75, 162, 0.4);
            }
            QPushButton:hover {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #768eea, stop:1 #865ba2);
                box-shadow: 0 6px 8px -1px rgba(102, 126, 234, 0.5), 0 4px 6px -1px rgba(118, 75, 162, 0.5);
            }
            QPushButton:pressed {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #566eea, stop:1 #663ba2);
                box-shadow: 0 2px 4px -1px rgba(102, 126, 234, 0.4), 0 1px 2px -1px rgba(118, 75, 162, 0.4);
            }
        """)
...
class FileItemWidget(QWidget):
    ...
    def __init__(self, file_info, file_manager=None):
        ...
        filename_label.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        filename_label.setMinimumWidth(200)
        ...
        header_layout.addLayout(info_layout, 1)  # 添加伸缩因子，让info_layout占据剩余空间
        ...
        if tags:
            ...
            for i, tag in enumerate(tags[:3]):  # 最多显示3个标签
                ...
                tag_label.setStyleSheet(style % (bg_color, text_color))
    ...
    def enterEvent(self, event):
        ...
        self.animation.setEndValue(self.pos() - QPoint(0, 4))
    ...
    def _get_file_icon(self, file_type):
        ...
        # 创建渐变
        from PyQt6.QtGui import QLinearGradient
        gradient = QLinearGradient(0, 0, 0, 64)  # 上下渐变
        ...
        painter.setFont(QFont('Times New Roman', 16, QFont.Weight.Bold))  # TIMES NEW ROMAN加粗字体
        
        # 根据文件类型添加不同的字母
        if file_type in ['xlsx', 'xls']:
            # Excel：添加'X'
            painter.drawText(42, 10, 20, 20, Qt.AlignmentFlag.AlignCenter, 'X')
        elif file_type in ['docx', 'doc']:
            # Word：添加'W'
            painter.drawText(42, 10, 20, 20, Qt.AlignmentFlag.AlignCenter, 'W')
        elif file_type in ['pptx', 'ppt', 'pdf']:
            # PPT和PDF：添加'P'
            painter.drawText(42, 10, 20, 20, Qt.AlignmentFlag.AlignCenter, 'P')
        elif file_type == 'txt':
            # TXT：添加'T'
            painter.drawText(42, 10, 20, 20, Qt.AlignmentFlag.AlignCenter, 'T')
...
class FileManagementWidget(QWidget):
    ...
    def __init__(self, file_manager, file_importer):
        ...
        category_label.setStyleSheet("color: #6b7280; font-size: 14px; font-family: '微软雅黑', 'Times New Roman';")
        ...
        view_label.setStyleSheet("color: #6b7280; font-size: 14px; font-family: '微软雅黑', 'Times New Roman';")
...
class TagManagementWidget(QWidget):
    ...
    def __init__(self, tag_manager, file_manager):
        ...
        """)
        # 设置标签列表的最小宽度
        self.tag_list.setMinimumWidth(150)
        # 设置标签列表的默认宽度
        self.tag_list.setMaximumWidth(300)
        ...
        # 设置文件列表的最小宽度
        self.tag_file_list.setMinimumWidth(400)
    ...
    def refresh_tags(self):
        ...
        try:
            ...
            print(f"刷新标签列表时出错: {str(e)}")
            # 获取主窗口的Toast通知实例
            main_window = self.window()
            if hasattr(main_window, 'toast'):
                main_window.toast.show_notification(f"刷新标签列表失败: {str(e)}", "error")
    ...
    def select_tag(self, item):
        ...
        for file in files:
            try:
                ...
                # 处理异常，避免因单个文件错误导致整个方法失败
                print(f"获取文件标签时出错: {str(e)}")
                ...
                if result and isinstance(result, dict) and 'success' in result:
                    if result['success']:
                        ...
                        if hasattr(main_window, 'toast'):
                            ...
                            main_window.toast.show_notification(f"标签添加失败: {message}", "error")
                else:
                    # 获取主窗口的Toast通知实例
                    main_window = self.window()
                    if hasattr(main_window, 'toast'):
                        main_window.toast.show_notification("标签添加失败: 无效的返回结果", "error")
        except Exception as e:
            print(f"添加标签时出错: {str(e)}")
            # 获取主窗口的Toast通知实例
            main_window = self.window()
            if hasattr(main_window, 'toast'):
                main_window.toast.show_notification(f"添加标签失败: {str(e)}", "error")
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                if result and isinstance(result, dict) and 'success' in result:
                    if result['success']:
                        ...
                        if hasattr(main_window, 'toast'):
                            message = result.get('message', '未知错误')
                            main_window.toast.show_notification(f"标签删除失败: {message}", "error")
                else:
                    # 获取主窗口的Toast通知实例
                    main_window = self.window()
                    if hasattr(main_window, 'toast'):
                        main_window.toast.show_notification("标签删除失败: 无效的返回结果", "error")
        except Exception as e:
            print(f"删除标签时出错: {str(e)}")
            # 获取主窗口的Toast通知实例
            main_window = self.window()
            if hasattr(main_window, 'toast'):
                main_window.toast.show_notification(f"删除标签失败: {str(e)}", "error")
    ...
    def add_file_to_tag(self):
        ...
        try:
            ...
            if not files:
                ...
                main_window = self.window()
                if hasattr(main_window, 'toast'):
                    main_window.toast.show_notification("没有可用文件", "warning")
                return
            ...
            for file in files:
                try:
                    ...
                    file_id = file.get('id', '未知ID')
                    category = file.get('category', '未分类')
                    file_names.append(f"{filename} (ID: {file_id})".ljust(50) + f"分类: {category}")
                except Exception as e:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted and selected_files:
                ...
                for file in selected_files:
                    try:
                        ...
                        if not file_id:
                            continue
                        result = self.tag_manager.add_tags_to_file(file_id, [tag_name])
                        if not (result and isinstance(result, dict) and result.get('success')):
                            success = False
                            # 获取主窗口的Toast通知实例
                            main_window = self.window()
                            if hasattr(main_window, 'toast'):
                                message = result.get('message', '未知错误') if result and isinstance(result, dict) else '添加失败'
                                main_window.toast.show_notification(f"添加文件到标签失败: {message}", "error")
                            break
                    except Exception as e:
                        print(f"添加文件到标签时出错: {str(e)}")
                        success = False
                        # 获取主窗口的Toast通知实例
                        main_window = self.window()
                        if hasattr(main_window, 'toast'):
                            main_window.toast.show_notification(f"添加文件到标签失败: {str(e)}", "error")
                        break
                if success:
                    # 获取主窗口的Toast通知实例
                    main_window = self.window()
                ...
                if isinstance(file_info, dict) and 'id' in file_info:
                    ...
                    if result and isinstance(result, dict) and 'success' in result:
                        if result['success']:
                            ...
                            if hasattr(main_window, 'toast'):
                                message = result.get('message', '未知错误')
                                main_window.toast.show_notification(f"从标签移除文件失败: {message}", "error")
                    else:
                        # 获取主窗口的Toast通知实例
                        main_window = self.window()
                        if hasattr(main_window, 'toast'):
                            main_window.toast.show_notification("从标签移除文件失败: 无效的返回结果", "error")
                else:
                    # 获取主窗口的Toast通知实例
                    main_window = self.window()
                    if hasattr(main_window, 'toast'):
                        main_window.toast.show_notification("无法获取文件ID", "error")
            else:
                # 获取主窗口的Toast通知实例
                main_window = self.window()
                if hasattr(main_window, 'toast'):
                    main_window.toast.show_notification("无法获取文件信息", "error")
        except Exception as e:
            # 处理异常，避免因错误导致整个方法失败
            print(f"从标签移除文件时出错: {str(e)}")
            # 获取主窗口的Toast通知实例
            main_window = self.window()
            if hasattr(main_window, 'toast'):
                main_window.toast.show_notification(f"操作失败: {str(e)}", "error")
...
class GraphView(QGraphicsView):
    ...
    def _on_node_click(self, node):
        ...
        if node_id in self.nodes:
            ...
            circle.setScale(1.3)
            # 缩小
            QTimer.singleShot(200, lambda: circle.setScale(1.0))
            # 再次放大
            QTimer.singleShot(400, lambda: circle.setScale(1.3))
            # 再次缩小
            QTimer.singleShot(600, lambda: circle.setScale(1.0))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # 设置窗口样式 - 使用back1.jpg作为背景
        back1_path = get_resource_path(os.path.join("RESOURCE", "back1.jpg"))
        
        # 如果找到背景图片，设置为背景
        if os.path.exists(back1_path):
            # 转换路径为URL格式
            back1_url = back1_path.replace("\\", "/")
            self.setStyleSheet(f"""QMainWindow {{
                background-image: url({back1_url});
        ...
        user_label.setStyleSheet("color: #6b7280;")
        # 添加到状态栏
        self.status_bar.addWidget(status_label, 1)
    ...
    def _setup_top_navigation(self):
        ...
        # 使用RESOURCE目录中的png图标
        pixmap_path = get_resource_path(os.path.join("RESOURCE", "1771260519474.png"))
        if os.path.exists(pixmap_path):
            pixmap = QPixmap(pixmap_path)
            # 调整图标大小以适应标签
        ...
        self.search_input.setStyleSheet("""
            ...
            QLineEdit:hover {
                border-color: #d1d5db;
                box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 2px 4px 0 rgba(0, 0, 0, 0.06);
            }
            QLineEdit:focus {
                border-color: #8b5cf6;
                box-shadow: 0 0 0 3px rgba(139, 92, 246, 0.1);
        ...
        main_layout.addLayout(search_layout, 1)  # 搜索框占据剩余空间
        ...
        user_layout.addWidget(notification_button)
        ...
        user_layout.addWidget(settings_button)
        
        # 用户头像
        self.user_avatar = QLabel()
        self.user_avatar.setFixedSize(40, 40)
        # 使用RESOURCE目录中的png图标
        avatar_path = get_resource_path(os.path.join("RESOURCE", "1761626953104.png"))
        if os.path.exists(avatar_path):
            avatar_pixmap = QPixmap(avatar_path)
            # 调整图标大小以适应标签
            avatar_pixmap = avatar_pixmap.scaled(40, 40, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
            self.user_avatar.setPixmap(avatar_pixmap)
        else:
            # 如果图标文件不存在，使用默认的蓝紫渐变头像
...
def check_temp_directory():
    ...
    try:
        ...
        # 测试是否可写
        test_file = os.path.join(test_dir, '.lfm_test_' + str(os.getpid()))
...
def check_system_requirements():
    ...
    try:
        ...
        # 只支持Windows系统
        if system != 'Windows':
            return False, f"不支持的操作系统: {system}，仅支持Windows"
        
        # 检查Python版本（虽然打包后不需要，但保留检查）
        if sys.version_info < (3, 8):
            return False, f"Python版本过低: {sys.version_info.major}.{sys.version_info.minor}，需要3.8或更高版本"
        
        # 检查临时目录
        temp_ok, temp_msg = check_temp_directory()
        if not temp_ok:
            return False, f"临时目录无法访问: {temp_msg}"
