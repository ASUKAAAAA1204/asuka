...
class DataStore:
    ...
    def get_files(self, category: Optional[str] = None, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in rows:
                file_info = self._row_to_file_info(row)
                # 添加标签信息
                file_info['tags'] = self.get_file_tags(file_info['id'])
    ...
    def get_file_tags(self, file_id: int) -> List[str]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # 限制缓存大小
            if len(self._tag_cache) > 100:
                # 删除最早的缓存项
                oldest_key = next(iter(self._tag_cache))
                del self._tag_cache[oldest_key]
    ...
    def search_files(self, query: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''
                ...
                LEFT JOIN tags t ON f.id = t.tag
                WHERE f.filename LIKE ? OR k.keyword LIKE ? OR t.tag LIKE ?
                ORDER BY f.added_at DESC
                LIMIT ?
