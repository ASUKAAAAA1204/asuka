...
class FileManager:
    ...
    def delete_file(self, file_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not os.path.normpath(file_path).startswith(os.path.normpath(self.storage_dir)):
                raise PathValidationError('安全检查失败：文件路径不在存储目录内')
            ...
            if not os.path.exists(file_path):
                raise FileNotFoundError('文件不存在或已被删除')
            
            try:
                shutil.move(file_path, recycled_path)
            except PermissionError:
                raise FileAccessError('权限不足，无法移动文件')
            except OSError as e:
                raise FileOperationError(f'文件移动失败: {str(e)}')
            ...
            return {'success': False, 'message': f'验证错误: {str(e)}'}
        except FileNotFoundError as e:
            return {'success': False, 'message': f'文件错误: {str(e)}'}
        except PathValidationError as e:
            return {'success': False, 'message': f'路径错误: {str(e)}'}
        except FileAccessError as e:
            return {'success': False, 'message': f'访问错误: {str(e)}'}
        except FileOperationError as e:
            return {'success': False, 'message': f'操作错误: {str(e)}'}
    ...
    def restore_file(self, file_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # 检查文件是否在回收站
            file_path = file_info['file_path']
            if not os.path.normpath(file_path).startswith(os.path.normpath(self.recycle_bin_dir)):
                raise PathValidationError('文件不在回收站中')
            ...
            return {'success': False, 'message': f'存储错误: {str(e)}'}
        except Exception as e:
            return {'success': False, 'message': f'未知错误: {str(e)}'}
    ...
    def permanently_delete_file(self, file_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # 安全检查：确保文件路径在存储目录或回收站中
            file_path = file_info['file_path']
            norm_file_path = os.path.normpath(file_path)
            norm_storage_dir = os.path.normpath(self.storage_dir)
            norm_recycle_dir = os.path.normpath(self.recycle_bin_dir)
            
            if not (norm_file_path.startswith(norm_storage_dir) or norm_file_path.startswith(norm_recycle_dir)):
                raise PathValidationError('安全检查失败：文件路径不在允许的目录内')
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    raise FileAccessError('权限不足，无法删除文件')
                except OSError as e:
    ...
    def move_file(self, file_id: int, new_category: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(file_id, int) or file_id <= 0:
                raise ValidationError('无效的文件ID')
            if not new_category or '..' in new_category or '/' in new_category or '\\' in new_category:
                raise ValidationError('无效的分类名')
    ...
    def copy_file(self, file_id: int, new_category: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise FileAccessError('权限不足，无法复制文件')
            except OSError as e:
                raise FileOperationError(f'文件复制失败: {str(e)}')
    ...
    def rename_file(self, file_id: int, new_filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.path.exists(new_path):
                raise FileOperationError('文件名已存在')
            ...
            try:
                ...
                raise FileAccessError('权限不足，无法重命名文件')
            except OSError as e:
                raise FileOperationError(f'文件重命名失败: {str(e)}')
    ...
    def export_file(self, file_id: int, export_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(file_id, int) or file_id <= 0:
                raise ValidationError('无效的文件ID')
            if not export_dir:
                raise ValidationError('导出目录不能为空')
            ...
            try:
                ...
                raise StorageError(f'创建导出目录失败: {str(e)}')
            ...
            try:
                ...
                raise FileAccessError('权限不足，无法导出文件')
            except OSError as e:
                raise FileOperationError(f'文件导出失败: {str(e)}')
    ...
    def create_backup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise StorageError(f'创建备份目录失败: {str(e)}')
            
            # 创建备份
            try:
                shutil.make_archive(os.path.splitext(backup_path)[0], 'zip', self.storage_dir)
            except PermissionError:
                raise FileAccessError('权限不足，无法创建备份')
            except OSError as e:
                raise BackupError(f'备份创建失败: {str(e)}')
            ...
            return {'success': False, 'message': f'备份错误: {str(e)}'}
        except Exception as e:
            return {'success': False, 'message': f'未知错误: {str(e)}'}
    ...
    def restore_backup(self, backup_path: str) -> Dict[str, Any]:
        ...
        try:
            # 验证输入
            if not backup_path:
                raise ValidationError('备份文件路径不能为空')
            ...
            if not backup_path.endswith('.zip'):
                raise ValidationError('无效的备份文件格式，只支持ZIP格式')
            ...
            try:
                ...
                raise StorageError(f'创建临时目录失败: {str(e)}')
            
            # 解压备份文件
            try:
                shutil.unpack_archive(backup_path, temp_dir)
            except shutil.ReadError:
                raise BackupError('备份文件损坏或格式不正确')
            except OSError as e:
                raise BackupError(f'解压备份文件失败: {str(e)}')
            ...
            if os.path.exists(backup_storage_dir):
                ...
                try:
                    ...
                    raise FileAccessError('权限不足，无法备份当前存储目录')
                except OSError as e:
                    raise StorageError(f'备份当前存储目录失败: {str(e)}')
                
                # 移动备份到存储目录
                try:
                    shutil.move(backup_storage_dir, self.storage_dir)
                except PermissionError:
                    raise FileAccessError('权限不足，无法恢复备份')
                except OSError as e:
                    raise StorageError(f'恢复备份失败: {str(e)}')
                ...
                try:
                    ...
                    pass
                raise BackupError('备份文件格式不正确，缺少存储目录')
        except ValidationError as e:
            return {'success': False, 'message': f'验证错误: {str(e)}'}
        except FileNotFoundError as e:
            return {'success': False, 'message': f'文件错误: {str(e)}'}
            ...
            for file_info in recycle_bin_files:
                if os.path.exists(file_info['file_path']):
                    try:
                        ...
                        raise FileAccessError(f'权限不足，无法删除文件: {file_info["filename"]}')
                    except OSError as e:
                        raise FileOperationError(f'删除文件失败: {file_info["filename"]} - {str(e)}')
                # 从数据库中删除文件
                try:
